...
//	cout << "<ul>" << endl;
	for (iModule = 0; iModule < Nmodules; iModule++)
	{
		...
		{
			outFilePS  = outputDir + "LED/"  + "r" + fRunNumber + "_" + moduleName[iModule] + "_DeadChannels.ps";
			outFilePNG = outputDir + "LED/"  + "r" + fRunNumber + "_" + moduleName[iModule] + "_DeadChannels.png";
		}
		else
		...
		if (strcmp(fRunType, "Led") == 0)
		{
			plotFilename = outputWebDir + "LED/" + basename(outFilePNG.Data());
			plotPsFilename = outputWebDir + "LED/" + basename(outFilePS.Data());
		}
		else
		if (strcmp(fRunType, "MonoCis") == 0)
		{
