...
{
	today();
	document.myForm.rday.value=document.myForm.resetDay.value;
	document.myForm.month.value=document.myForm.resetMonth.value;
	document.myForm.year.value=document.myForm.resetYear.value;
	document.myForm.run.value="0000000";
	document.myForm.run2.value="9999999";
	document.myForm.opr_run.selectedIndex=3;
	document.myForm.opr_run2.selectedIndex=4;
	document.myForm.opr_date.selectedIndex=3;
	document.myForm.opr_date2.selectedIndex=4;
	document.myForm.rows.value =25;
	document.myForm.orderdir.value="desc";
	document.myForm.exp_type.selectedIndex=0;
	document.myForm.exp_setup.selectedIndex=0;
...
<?php
		$con = mysql_connect("pcata007","tilecal","")
			or die("cannot connect to database server pcata007 :(");
		$select = "SELECT DATE_FORMAT(date, '%Y'), DATE_FORMAT(date, '%m'), DATE_FORMAT(date, '%d')  FROM comminfo ORDER BY date ASC";
		mysql_select_db("tile", $con);
		$res = mysql_query($select, $con)
			or die("query failed :(");
		mysql_data_seek($res, 2);
  ...
  echo "</SELECT>";
  mysql_close();
  	echo "<td><input name=\"resetYear\" type=\"hidden\" size=\"4\" value=\"$year\" maxlength=\"4\"></td>\n";
	echo "<td><input name=\"resetMonth\" type=\"hidden\" size=\"2\" value=\"$month\" maxlength=\"2\"></td>\n";
	echo "<td><input name=\"resetDay\" type=\"hidden\" size=\"2\" value=\"$day\" maxlength=\"2\">\n</td><td>";
?>
</td>
</tr>
<tr>
