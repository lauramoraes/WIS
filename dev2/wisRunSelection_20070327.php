...
<title>TileCal Commissioning Offline Shifts</title>
		...
		function ntuplePerDrawer(moduleString, runNumber, runType)
	{
			...
			popup=window.open(URL, "barrels" , 'width=780,height=1200,Scrollbars=YES,Resizable=YES');
			if(window.focus)
				popup.focus();
	}
...
//Accessing comminfo
		...
		{
			$select = "SELECT run, type, DATE_FORMAT(date,'%d/%m/%Y %H:%i:%s'), events, digifrags FROM comminfo WHERE (trigger IS NULL or strcmp(trigger,'BAD')!=0) ORDER BY date DESC"; //LIMIT 0,10";
			$rows = 30;
		}
...
?><BR/>
	...
	<script type="text/javascript">
      ...
      <table id="mainTable" class="main" border="0" cellpadding="2" cellspacing="1" align="center" width="100%">
        <tr> 
          ...
          <th noWrap name="dateData" id="dateData" rowspan="2">Run Date <A HREF="javascript:closeOrOpenCol('dateData')"> <IMG BORDER="0" SRC="images/xicon2.gif"/></a> </th>
          ...
          <th noWrap name="overviewData" id="overviewData" rowspan="1" colspan="5"> Overview  <A HREF="javascript:closeOrOpenCol('overviewData')"> <IMG BORDER="0" SRC="images/xicon2.gif"></a> </th>
	...
	<th name="overviewData" id="overviewData" width="40 px"><font size="1" face="Arial, Helvetica, sans-serif">Not Analized</font></th>
        </tr>
        <?php
