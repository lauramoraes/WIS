...
int main(int argc, char *argv[])
{
	...
	//if (pid == 0) // child process
	{
		...
		snprintf(execFilename, MAX_LENGTH_FILENAME + 1, "./macro/TTileCommRun/checkDeadChannels");
		//printf("%s\n", execFilename);
		...
		//snprintf(argvRoot[4], MAX_LENGTH_STRING + 1, "./macro/checkDeadChannels.cpp(\"%s\", \"\", \"%s\", \"%s\")", runNumber, runType, module);
		//for (i = 0; i < 4; i++)
			//printf("%s\n", argvRoot[i]);
