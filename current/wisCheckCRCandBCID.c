...
{
	char runNumber[MAX_LENGTH_STRING + 1], runType[MAX_LENGTH_STRING + 1], barrel[MAX_LENGTH_STRING + 1];
	...
	logtest=fopen("./logtest.log", "w");
	fprintf(logtest, "entrou aki\n");
	pid = fork();
	
	if (pid < 0 )
	{
	fprintf(logtest, "entrou aki0\n");
		printf("error id=%d\n",pid);
		fclose(logtest);
		exit(1); 
	}
	...
	{
		fprintf(logtest, "entrou parent processs\n");
	...
	//int  execve(const  char  *filename,  char  *const  argv [], char *const envp[]);
		...
		snprintf(execFilename, MAX_LENGTH_FILENAME + 1, "./macro/TTileCommRun/checkCRC_DMU_BCID");
		//printf("%s\n", execFilename);
		for (i = 0; i < 3; i++)
		...
		//snprintf(argvRoot[4], MAX_LENGTH_STRING + 1, "./macro/checkCRCandBCID.cpp(\"%s\", \"\", \"%s\", \"%s\")", runNumber, runType, module);
	fprintf(logtest,"%i\n", retcode);
		for (i = 0; i < 3; i++)
		fprintf(logtest, "%s ", argvRoot[i]);
...
//		retCode = execve(execFilename, argvRoot, envRoot);
		retCode = execve(execFilename, argvRoot, envp);
//	retCode = execve(execFilename, argvRoot, NULL);
