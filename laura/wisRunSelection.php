...
//Accessing comminfo
		...
		{
			$select = "SELECT run, type, DATE_FORMAT(date,'%d/%m/%Y %H:%i:%s'), events, setup, digifrags FROM comminfo WHERE (trig IS NULL or strcmp(trig,'BAD')!=0) ORDER BY date DESC"; //LIMIT 0,10";
			$rows = 30;
		}
...
?><BR/>
	...
	{
		global $orderdir, $select, $orderedby,$start;
		$pos=strpos($select , "ORDER");
		$tmp_select=substr( $select , 0, $pos);
		$tmp_orderdir="DESC";
		$URL=$_SERVER["PHP_SELF"]."?select=".$tmp_select." ORDER BY ";
		$endURL="&rows=30&start=".$start."&query=query";
		$sufix1="&orderdir=";
		$sufix2="&orderedby=";
		if($string=="Run")
		{
		...
		if ($orderedby==$value)
		{
			...
			$image="<IMG BORDER=\"0\" SRC=\"images/arrow$dir.JPG\"/>";
		}
		else $image="";
		$URL.=$value;
		$URL.="+".$tmp_orderdir.$endURL.$sufix1.$tmp_orderdir.$sufix2.$value;
		echo "<A HREF=\"$URL\">$image $string</A>";
...
for ($i = $start; $i <= $end; $i++)
{
	...
	{
		//print_r($runOKArray);
		echo"<font color=\"green\">".$perc_array[0]."%</font>";
	}
	echo"</td>";
	echo"<td align=\"center\" name=\"overviewData\" id=\"overviewData\">";
	if($Nmodules!= 0)
	{
		echo"<font color=\"#FBB117\">".$perc_array[1]."%</font>";
	}
	...
	if($Nmodules!= 0)
	{
		echo"<font color=\"red\">".$perc_array[2]."%</font>";
	}
	...
	if($Nmodules!= 0)
	{
		echo"<font color=\"black\">".$perc_array[3]."%</font>";
	}
	...
	{
	$selectPlot="http://tcws.web.cern.ch/tcws/TA/TileCommAnalysis/version1.4/searchResult.php?runNumber=".$row[0];
...
<?php
  ...
  echo "Pages: ";
  $sufix="&orderdir=".$orderdir."&orderedby=".$orderedby;
  $p = $start - $rows;
  $prev = $curr . $p.$sufix;
